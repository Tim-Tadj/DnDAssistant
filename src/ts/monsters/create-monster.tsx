...
const CreateMonster: FC<{ onCreated?: () => void }> = ({ onCreated }) => {
  ...
  const onSave = async () => {
    if (!currentMonster || !currentMonster.name.trim()) {
    ...
    setSubmitError(null);
    try {
      await monstersApi.create(currentMonster);
      setToastOpen(true);
      onCreated?.();
      close();
    } catch (e) {
          ...
          <Card sx={{ flex: 1, flexGrow: 1, maxWidth: "lg" }}>
            <CardContent>
              <Stack spacing={2} direction="row" justifyContent="space-between" useFlexGap maxWidth="auto">
                <Box flexGrow={1}>
                  <MonsterEditor
                    onUpdateGear={setNewMonsterJSON}
                    onChange={setCurrentMonster}
                  />
                </Box>
