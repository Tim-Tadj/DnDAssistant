...
-- Monsters: derived from the Monster Manual (provenance='derived') plus
-- user-created homebrew. The columns mirror the bundled JSON shape from
-- src/res/resources/monster_manual_monsters.json; ability scores are kept
-- as text (the original values include "+5" / "(+5)" forms) so we don't
-- lose fidelity. Numeric CR/XP are derived in the encounter generator.
...
-- Phase 5 will create a `users` table. We don't model it yet; ownership
